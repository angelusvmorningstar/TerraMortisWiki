...
## Status API (story 3-2)

- `GET /api/status` — the assembled City / Covenant / Clan standing ladders, read live from Mongo (behind `requireAuth`, mounted after `app.use(requireAuth)` in `server/index.js` alongside the characters and world routers). Returns `{ city: { rows }, covenant: { ladders: [{ name, rows }] }, clan: { ladders: [{ name, rows }] } }`, where each row is `{ _id, name, honorific, moniker, value, mine }`.

**This is the SECOND per-viewer authorisation boundary in the repo (after `server/routes/characters.js`), and the FIRST route to read and return any character `status` field (`status.city` / `status.covenant` / `status.clan`).** `getCharacters()` hands `buildStatusView` full, unredacted documents, including every `status.covenant` key for every covenant a character holds standing in. The covenant/clan gating in `server/routes/status.js` is the ONLY place the redaction happens; a mistake leaks one faction's internal standing to every logged-in player, including rival factions. Three secret channels are gated, all asserted in `server/routes/status.test.js`:

- **City Status is ungated** — every logged-in viewer sees every non-retired character, valued at the RAW stored `status.city` (deliberately NOT TM Suite's computed title + ambience total; a documented simplification to keep numeric-correctness risk off this access-control story).
- **Covenant / Clan Status are gated per viewer.** The covenant list is the UNION across the viewer's owned character(s) of each character's primary `covenant` plus any covenant where it holds `status.covenant[cov] > 0`; the clan set is the union of the owned characters' `clan` values. A ladder for a faction the viewer does not belong to is **never assembled** — the gating happens server-side, before the response leaves Express, not by hiding on the client.
- **Each ladder row exposes ONE scalar (`value`), never the `status` sub-document.** A character may hold standing in several covenants; exposing the whole `status.covenant` map inside a row would leak their standing in covenants the viewer is not entitled to see. Rows are built by **allowlist construction** (`statusRow`) — a NEW object from named fields only, never a `{ ...character }` or `{ ...character.status }` spread — so a field added to the `characters` schema upstream can never silently leak.

Retired characters are excluded from all ladder ROWS, but the viewer's own covenant/clan LISTS are derived from their owned character(s) whether retired or not (a retired-only owner still sees their faction's current ladder). A spectator (empty `character_ids`) gets City Status in full and empty Covenant/Clan sections — fail-closed, never "everything". The HTTP-level [LEAK-GATE] tests assert the serialised body carries no other faction's ladder, no `status` sub-document, and no owner-only field; a discrimination negative-control proves a passthrough regression trips them. A later story may reuse this boundary rather than re-deriving the covenant/clan gating.
